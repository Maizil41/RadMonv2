...
<i class="fa fa-print pointer" title="Print Batch ${batch.batch_name}" onclick="openPrintWindow('${batch.plan_name}', '${accountsStr}', '${batch.batch_name}', ${batch.total_user})"></i>
...
function openPrintWindow(planName,accountsStr,batchName,totalUser){if(totalUser===0){alert("Tidak ditemukan kode voucher untuk batch ini");return}
...
const url=`./${selectedOption}?type=batch&plan=${planName}&accounts=Username,Password||${accountsStr}`;const newWindow=window.open(url,"_blank");newWindow.onload=function(){newWindow.print()}}
