...
function printTicketsHTMLTable(array $accounts, string $ticketCostText, int $ticketCostNum, string $ticketTime, string $ticketQuota, string $ticketActiveTime, string $timestamp, array $cfg)
{
    ...
    .rotate {vertical-align: middle; text-align: center; font-size: 20px;}
    .rotate span {-ms-writing-mode: tb-rl; -webkit-writing-mode: vertical-rl; writing-mode: vertical-rl; transform: rotate(180deg); white-space: nowrap;}
    </style>';
...
?>
<table style="display:inline-block;border-collapse:collapse;border:1px solid #666;margin:2.5px;width:230px;overflow:hidden;position:relative;padding:1px;border:1px solid <?= esc($color) ?>;">
  <tr>
    <td class="rotate" rowspan="4" style="font-size:13.5px;font-weight:bold;color:#fff;background-color:<?= esc($color) ?>;-webkit-print-color-adjust:exact;">
      <span>Rp.<?= esc($ticketCostText) ?></span>
    </td>
    ...
    <?php elseif ($logomode === "image"): ?>
      <td>
        <center><img src="../img/logo/radmon-logo.png" alt="logo" style="height:20px;width:70px;border:0;"></center>
      </td>
    <?php else: ?>
      <td></td>
    <?php endif; ?>
    ...
    <td rowspan="4">
      <img src="<?= esc($qrSrc) ?>" alt="QR Code" style="border:2px solid <?= esc($color) ?>;width:65px;height:65px;">
    </td>
  </tr>

  <tr>
    <td style="width:100%;font-weight:bold;font-size:18px;color:#111;text-align:center;"><?= esc($user) ?></td>
  </tr>

  <tr>
    <td style="font-weight:bold;font-size:10px;color:#000;">
      <center><?= esc(trim($ticketActiveTime . ' ' . $ticketTime . ' ' . $ticketQuota)) ?></center>
    </td>
  </tr>

  <tr>
    <td style="font-weight:bold;font-size:10px;color:#000;">
      <center>CS : <?= esc($formatted_cs) ?></center>
    </td>
  </tr>
</table>
<?php
